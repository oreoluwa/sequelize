...
const debug = logger.debugContext('sql:rqlite');
...
class Query extends AbstractQuery {
  ...
  _handleQueryResponse(columnTypes, err, formattedResults) {
    const results = formattedResults.toArray().filter(r => !_.isEmpty(r))
    ...
    const dataResult = formattedResults.get(0);
    const metaData = {
      sql: this.sql,
      changes: dataResult.getRowsAffected(),
      lastId: dataResult.getLastInsertId(),
    };
    ...
    if (this.isSelectQuery()) {
      ...
      return this.handleSelectQuery(selectResults);
    }
      ...
      const columnTypes = {};
        const executeSql = () => {
          ...
          resolve(new Promise((resolve, reject) => {
            const query = this;
            conn[method](sql).then(dataResults =>
              resolve(
                query._handleQueryResponse(
                  columnTypes,
                  dataResults.getFirstError(),
                  dataResults
                )
              )
            ).catch(reject);
        ...
        return Promise.map(tableNames, tableName =>
          new Promise(resolve => {
            ...
            conn[method](`PRAGMA table_info(\`${tableName}\`)`).then(dataResults => {
              const results = dataResults.toArray();
